(function () {
  ...
  async function buildPrompt(context) {
    try {
      ...
      return template.replace('{{CONTEXTO}}',JSON.stringify(context,null,2)).replace('{{CONTRATO}}','```json\n'+JSON.stringify(Native.emptyInitialReport(context.perfil,context.inicio,context.fim,currentModel===Native?currentReference:Native.initialReference()),null,2)+'\n```').replace('{{VALIDATOR}}',()=> '```javascript\n'+validator.join('\n')+'\n```');
  ...
  async function completeAnalysis(data) {
    ...
    try {
      const prompt=await buildPrompt({perfil:data.profile,inicio:data.window.start,fim:data.window.end,modo:'retomar métricas nativas nos artefatos existentes',ler_conteudo_de_mensagens:false});
      if(revision!==reportGeneration)return;
      const isNative=data.method===Native.METHOD;
      const base=isNative?'Reutilize este JSON nativo como dado de entrada. Preserve seus valores comprovados.\n\n<dados_existentes>\n'+JSON.stringify(data,null,2)+'\n</dados_existentes>\n\n':'O arquivo anterior usa métricas diferentes. Reaproveite somente os campos nativos dos artefatos privados da coleta, não as taxas de subconjuntos Creator/Expert/Founder. Não converta as antigas classificações semânticas em métricas nativas.\n\n';
  ...
  function renderReport(data,demo,override,prepared) {
    ...
    const viewModel=imported.data.method===Native.METHOD?Native:ECF;
