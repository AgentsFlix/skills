...
4. **Novos seguidores da conta:** na mesma rota, consulta separada com `metrics=follows_and_unfollows`, `metricType=total_value`, `breakdown=follow_type`. Leia o bloco da métrica `follows_and_unfollows`, com `breakdown=follow_type`. Nesse contexto específico, a dimensão **FOLLOWER** representa seguimentos brutos e **NON_FOLLOWER** representa saídas. No formato documentado do Zernio, selecione em `metrics.follows_and_unfollows.breakdowns` a única entrada com `dimension === "FOLLOWER"` e copie seu `value` inteiro para `account.follows`. Confira o envelope real de conta, período, métrica e breakdown antes da extração. A comparação deve ser exata: não use substring, pois NON_FOLLOWER também contém FOLLOWER; não selecione pelo índice da lista. Não rejeite FOLLOWER por não se chamar FOLLOWS. Não aplique esse mapeamento a `reach`, `views`, audiência ou outra métrica, onde as dimensões têm outro significado. Entrada ausente, duplicada ou valor inválido mantém null e motivo; UNKNOWN não é seguimento. Não use total de entradas/saídas, saldo líquido ou follows de posts no lugar. `followers_gained` da rota de histórico é soma de deltas diários positivos, não contagem de todos os seguimentos: não é fallback para esta variável. Se a dimensão de seguimentos não for retornada, account.follows=null e motivo em coverage.
5. **Inbox agregado:** `GET /v1/analytics/inbox/volume`, fromDate/início, toDate/fim, `platform=instagram`, accountId e profileId da conta. Leia **summary.uniqueConversations** da janela inteira. Não some faixas ou dias; não use sent, received ou total de pessoas como substituto. Zero nessa rota significa zero no agregado de eventos retornado para os filtros; não comprova ausência de DMs no Instagram. Se respostas anteriores da mesma janela indicarem conversas, confira os IDs/filtros e documente a cobertura de eventos do agregado; não converta a contagem antiga em uniqueConversations nem apresente as duas superfícies como equivalentes. Não filtre source para criar um universo diferente. Essa contagem é de conversas registradas pelo inbox, não de pessoas únicas, mensagens recebidas ou leads qualificados. Não use `/v1/inbox/conversations`, `/messages` ou `/comments` para completar este diagnóstico.
