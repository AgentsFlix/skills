...
(() => {
  ...
  function renderPuzzle() {
    ...
    $('selection').textContent = selected ? `Peça selecionada: ${getPiece(selected).text} Agora escolha uma etapa.` : 'Escolha uma peça na mesa abaixo. Para mover uma peça colocada, clique nela e depois na nova etapa.';
    $('slots').innerHTML = stages.map((stage, i) => {
      const piece = getPiece(slots[i]);
      const correct = piece?.stage === i;
      return `<div class="slot ${checked ? correct?'correct':'wrong' : ''}"><h3>${i+1} · ${stage}</h3><button class="drop ${piece?'':'empty'} ${piece?.id===selected?'selected':''}" data-slot="${i}" aria-label="${stage}: ${piece?piece.text:'espaço vazio'}">${piece?piece.text:'+ Colocar uma peça'}</button>${piece?`<button class="remove" data-remove="${i}" aria-label="Retirar peça de ${stage}">Retirar peça</button>`:''}${checked?`<p class="slot-hint">${correct?'✓ Correto':hints[i]}</p>`:''}</div>`;
    }).join('');
    $('tray').innerHTML = order.filter(id => !slots.includes(id)).map(id => `<button class="piece" data-piece="${id}" aria-pressed="${id===selected}">${getPiece(id).text}</button>`).join('');
  }
  function place(i) {
    if (!selected) {
