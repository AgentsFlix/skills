...
const art=n=>`<img src="art/${['academia','cafe','editor','loja','mensagem','profissional'].includes(n)?'v2':'eugencia'}/${n}.png" alt="" draggable="false">`;
const stages=[
...
function openFile(i){window.EpisodeSound?.play('document');$('file-title').textContent=stages[i].file;$('file-content').innerHTML=`${i===7?`<div class="file-previews">${a.templates.map(id=>sample(a.look,id,a.titles?.[id]||find(models,id).title)).join('')}</div>`:i===8?calendar():''}<pre>${esc(fileText(i))}</pre>`;$('file-dialog').showModal();$('close-file').focus();}
