...
function render(){
 ...
 $('steps').innerHTML=names.map((n,i)=>`<button class="step" data-step="${i}" ${i===step?'aria-current="step"':''}><strong>${i+1} · ${n}</strong><small>${subtitles[i]}</small></button>`).join('');
 ...
 `<div class="tool-card"><strong>Você</strong><p>Garçom + chef</p><p>Uma pessoa acompanha o pedido do começo ao fim.</p></div>`,
 `<span class="interaction-label">O que o cliente quer?</span><div class="choices"><button class="choice" data-tool="caderneta" aria-pressed="${tool==='caderneta'}">Pedir um miojo</button><button class="choice" data-tool="maquininha" aria-pressed="${tool==='maquininha'}">Pedir a conta</button></div><div class="tool-card"><strong>${tool==='caderneta'?'Caderneta':'Maquininha'}</strong><p>${tool==='caderneta'?'Você anota o prato e a mesa do cliente.':'Você usa a maquininha para cobrar a conta.'}</p></div>${tool==='maquininha'?'<p class="hint">A conta usa outra ferramenta. Para continuar esta simulação de preparo, volte ao pedido de miojo.</p>':''}`,
 `<span class="interaction-label">Menu de exemplo · escolha o tempero</span>${choices(['Frango','Carne','Legumes'],flavor,'flavor')}<div class="tool-card"><strong>Miojo de ${flavor.toLowerCase()}</strong><p>Pedido anotado para a mesa ${table}.</p></div>`,
 ...
 `<div class="delivery"><strong>${delivered?'Servido!':'Destino: mesa '+table}</strong><p>Miojo de ${flavor.toLowerCase()}, num prato elegante.</p><button class="primary" data-action="serve" ${delivered?'disabled':''}>${delivered?'Pedido entregue':`Servir na mesa ${table}`}</button></div>`
 ];
