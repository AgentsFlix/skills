...
function render(){
 ...
 const descriptions=['Escolha o negócio do novo cliente.','Compare a mesma mensagem em três vozes.','Compare cor, tipografia e composição.','Escolha 3 dos 6 modelos. Eles usam o visual que você definiu.'];
 let cards='';
 ...
 html=`<div class="capture-head"><p class="eyebrow">4 · ELICITAR</p><nav class="part-nav" aria-label="Partes da captura">${parts.map((name,i)=>`<button id="part-${i}" data-part="${i}" ${i===part?'aria-current="step"':''} ${i>0&&!ready().slice(0,i).every(Boolean)?'disabled':''}>${ready()[i]?'✓':i+1} ${name}</button>`).join('')}</nav><h2 id="question">${titles[part]}</h2><p>${descriptions[part]}</p></div>${part===3?`<p class="template-count" role="status">${profile.templates.length} / 3 escolhidos${profile.templates.length===3?' · Para trocar, retire um primeiro.':''}</p>`:''}<div class="choices-grid ${part===3?'template-catalog':''}">${cards}</div><div class="capture-foot"><button class="text-button" id="back">← Voltar</button><button class="primary" id="continue" ${!ready()[part]?'disabled':''}>${part===3?'Criar banco de templates':'Continuar'} →</button></div><details><summary>Por que elicitar?</summary><p>Para um cliente novo, a agência ainda não tem essas respostas. Você conversa, apresenta referências e registra as escolhas com ele. Neste exercício, as opções simulam essa conversa.</p></details>`;
 }
