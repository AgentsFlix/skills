...
function sample(look,kind='novidade',n=niche(),customTitle=null){
 const title=customTitle||(kind==='novidade'?n.subject:kind==='dica'?'Uma dica para hoje':'Por trás da marca');
...
document.addEventListener('click',e=>{
 ...
 else if(b.dataset.choice){invalidate();const key=b.dataset.choice,value=b.dataset.value;if(key==='templates')profile.templates=profile.templates.includes(value)?profile.templates.filter(v=>v!==value):[...profile.templates,value];else profile[key]=value;}
 ...
 else if(b.id==='continue'||b.id==='advance'||b.id==='back'||b.id==='review'||b.id==='pull'||b.id==='build'||b.id==='send'||b.dataset.go!==undefined||b.dataset.part!==undefined){$('question').tabIndex=-1;$('question').focus({preventScroll:true});}
});
