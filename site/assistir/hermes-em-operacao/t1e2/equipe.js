...
(() => {
  ...
  function renderAssembly(){
    ...
    $('lanes').innerHTML=[0,1,2].map(n=>`<article class="lane"><div class="lane-head">${chef}<div><h3>Chef ${n+1}</h3><p>${['Atendimento e entrega','Preparo na cozinha','Fechamento da conta'][n]}</p></div></div>${[n*2,n*2+1].map(i=>{const c=card(slots[i]);const ok=c?.slot===i;return `<div class="responsibility ${checked?(ok?'correct':'wrong'):''}" data-destination="${i}"><span class="role">${roles[i]}</span>${StageIdentity.badges([[0,1,2,3],[0,5],[0,1],[4,5],[0,1],[3,4,2,5]][i])}<button class="target ${c?'filled':''} ${c?.id===selected?'selected':''}" data-slot="${i}" draggable="${Boolean(c)}" ${c?`data-drag="${c.id}"`:''} aria-label="Chef ${n+1}, ${roles[i]}: ${c?c.text:'espaço vazio'}">${c?c.text:'+ Colocar uma peça'}</button>${c?`<button class="remove" data-remove="${i}">Retirar peça</button>`:''}${checked?`<p class="hint">${ok?'✓ Correto':hints[i]}</p>`:''}</div>`;}).join('')}</article>`).join('');
