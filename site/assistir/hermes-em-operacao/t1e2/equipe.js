...
(() => {
  ...
  const chef = n => StageIdentity.chef(n);
  ...
  function renderAssembly(){
    ...
    $('lanes').innerHTML=[0,1,2].map(n=>`<article class="lane"><div class="lane-head">${chef(n)}<div><h3>Chef ${n+1}</h3><p>${['Atendimento e entrega','Preparo na cozinha','Fechamento da conta'][n]}</p></div></div>${[n*2,n*2+1].map(i=>{const c=card(slots[i]);const ok=c?.slot===i;return `<div class="responsibility ${checked?(ok?'correct':'wrong'):''}" data-destination="${i}"><span class="role">${roles[i]}</span>${StageIdentity.badges([[0,1,2,3],[0,5],[0,1],[4,5],[0,1],[3,4,2,5]][i])}<button class="target ${c?'filled':''} ${c?.id===selected?'selected':''}" data-slot="${i}" draggable="${Boolean(c)}" ${c?`data-drag="${c.id}"`:''} aria-label="Chef ${n+1}, ${roles[i]}: ${c?c.text:'espaço vazio'}">${c?StageIdentity.piece(c.id)+c.text:'+ Colocar uma peça'}</button>${c?`<button class="remove" data-remove="${i}">Retirar peça</button>`:''}${checked?`<p class="hint">${ok?'✓ Correto':hints[i]}</p>`:''}</div>`;}).join('')}</article>`).join('');
    $('pieces').innerHTML=order.filter(id=>!slots.includes(id)).map(id=>`<button class="piece" data-piece="${id}" data-drag="${id}" draggable="true" aria-pressed="${id===selected}">${StageIdentity.piece(id)}${card(id).text}<small>${card(id).tags}</small></button>`).join('');
    $('connections').innerHTML=bridges.map((b,i)=>`<div class="connection"><label for="bridge-${i}">${b.label}</label><p>${b.sub}</p>${StageIdentity.badges(i===2?[0]:[5,0])}<select id="bridge-${i}" data-bridge="${i}"><option value="">Escolha o que passa</option>${options.map(([value,text])=>`<option value="${value}" ${links[i]===value?'selected':''}>${text}</option>`).join('')}</select>${checked?`<p class="hint">${links[i]===b.answer?'✓ Passagem correta':b.hint}</p>`:''}</div>`).join('');
  }
  function place(i){
  ...
  function renderSimulation(){
    ...
    $('team').innerHTML=[0,1,2].map(n=>`<article class="team-chef ${[incoming.length||serving.length,cooking.length,paying.length][n]?'active':''}"><div class="lane-head">${chef(n)}<div><h3>Chef ${n+1}</h3><p>${['Atende e serve','Prepara os pratos','Fecha as contas'][n]}</p></div></div><div class="metrics">${metrics[n].map(([value,label])=>`<div><strong data-metric="${label}">${value}<small> / ${states.length}</small></strong><span>${label}</span></div>`).join('')}</div><p class="current-job">${jobs[n]}</p></article>`).join('');
    $('orders').innerHTML=states.map((s,i)=>`<div class="team-order ${s===5?'done':s?'working':''}" aria-label="Pedido ${i+1}: ${phaseLabels[s]}">${StageIdentity.art(s>=4?'maquininha':s>=2?'prato':s===1?'caderneta':'cliente','order-art')}<b>${i+1}</b><span>${phaseLabels[s]}</span></div>`).join('');
