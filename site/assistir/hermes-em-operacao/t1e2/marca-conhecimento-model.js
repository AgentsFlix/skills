(function(root,factory){
  ...
  function parse(raw){
    ...
    const distribution=table(7,'Papel').map(row=>({role:get(row,'Papel'),label:get(row,'Proporção padrão'),ratio:proportion(get(row,'Proporção padrão')),purpose:get(row,'Função'),metric:get(row,'Métrica primária')}));
