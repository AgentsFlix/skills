(() => {
  ...
  const illustrations=new Set(['creator','expert','founder','audience','positioning','voice','entry']);
  const art=name=>illustrations.has(name)?'<img src="art/dashboard/'+name+'.png" alt="" aria-hidden="true" loading="eager" decoding="async">':'';
  ...
  function roleCards(mode){return '<div class="role-grid">'+roles.map(([name,currency,symbol])=>{
    ...
    const bar=mode==='plan'&&isRatio?'<span class="bar status-'+state+'" style="--ideal-dot: '+target+'" aria-hidden="true">'+Array.from({length:10},(_,i)=>'<i class="'+(i<amount?'filled':'')+'"></i>').join('')+'<b class="ideal-marker">⌃</b></span>':'';
