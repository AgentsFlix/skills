# Hermes em Operação · T1E3 · Método ECF

Capítulo interativo em `/assistir/hermes-em-operacao/t1e3/`. A jornada começa na explicação do método, gera o prompt de coleta, calcula o diagnóstico, conduz as seis etapas da Base ECF e termina no dashboard e na visão final da marca. O catálogo registra esta experiência como o capítulo 3 da Temporada 1.

O diagnóstico leva para a Base ECF somente as três notas, a quantidade de medições e a indicação de cobertura parcial. O relatório completo não é persistido. O resumo reduzido fica em `sessionStorage` até existir uma base, é validado por `base-editorial-ecf-scores.js`, salvo junto ao projeto e removido da sessão.

## Parte 1 · Diagnóstico ECF: métricas nativas do Zernio

Apresenta três cards, nove barras, médias e régua ajustável. O método atual é **ecf-zernio-v4**, solicitado em 10/09/2026 para usar somente métricas disponibilizadas pelo Zernio. A classificação de posts, comentários e DMs e a pesquisa de percepção deixam de ser pré-requisitos. A pedido do usuário, v4 substitui Novos seguidores por Contas engajadas para usar uma contagem nativa de conta sem interpretação de breakdown.
...
## Partes 2 a 5 · Base e dashboard

Depois do diagnóstico, `base-editorial.html` conduz Negócio, Pesquisa, Público, Posicionamento, Voz e Matéria-prima. Cada etapa copia um prompt para o Hermes e valida o JSON devolvido. Com 6/6 arquivos, o dashboard reúne o diagnóstico e os registros. O handoff seguinte gera a Base de Conhecimento Social Media e `minha-marca.html` abre Plano, Estratégia e Biblioteca.

Executar a suíte do repositório, check_site, validação/scanner de skills, build_docs sem diferença, agent_work check e QA em Chrome real em 1440, 768 e 390 px. Capturas de demonstração ficam em `design-review/base-dashboard/t1e3/`; dados reais ficam locais.
