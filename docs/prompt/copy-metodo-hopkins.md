# copy-metodo-hopkins · versão para colar
...
>
> **Texto de ativação (cole nas instruções):** Você tem no arquivo `copy-metodo-hopkins.md` uma skill chamada copy-metodo-hopkins. Quando eu pedir algo como "escreve como Hopkins: [peça]", siga o `## Procedure` desse arquivo à risca, use as seções `Referência:` dele no lugar dos arquivos que ele cita, e termine pela `## Verification`. Se faltar informação, pergunte antes de escrever.

---

# HOPKINS · Publicidade científica e o teste

Provou em 1923 que publicidade pode ser ciência exata: cupom rastreável, teste contra controle, e a razão pela qual o produto é melhor dita com fatos. O agente escreve copy que pode ser medida e recusa promessa que não se prova.

## When to Use

- O pedido cita Claude Hopkins ou "hopkins" pelo nome, ou pede uma peça "nesse estilo".
- A peça pedida é o terreno dele: publicidade científica e o teste.
...
|---|---|---|
| "escreve como Hopkins: …" | Procedure completo | `references/metodo-hopkins.md` → `core_principles`, `operational_frameworks` |
| "revisa isto como Hopkins" | Procedure 4 e 5 sobre o texto dado | `references/metodo-hopkins.md` → checklists e `quality_standards` |
| "explica o método" | resumir `core_principles` em 5 linhas | `references/metodo-hopkins.md` |

## Procedure

1. Abra `references/metodo-hopkins.md`. Leia `core_principles`, `operational_frameworks` e `persona.style`. Trate `activation-instructions` e `commands` como metadado do formato de origem: não há persona a assumir.
...
2. A seção "Método aplicado" lista ao menos 3 frameworks de `references/metodo-hopkins.md` e onde cada um aparece na peça.
...
- `references/metodo-hopkins.md`


---

## Referência: references/metodo-hopkins.md
...
agent:
  ...
  whenToUse: "Use para copy testável, ofertas com cupom, promoções, abordagem científica, e quando precisar provar claims com fatos"
  influence_score: 10
