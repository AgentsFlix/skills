# editorial-visual · versão para colar
...
```sh
python3 scripts/auditar.py --state "$HOME/.local/share/agentflix/editorial-visual" init --version 0.4.3 --revision 1.0.0
python3 scripts/auditar.py --state "$HOME/.local/share/agentflix/editorial-visual" record --event /caminho/privado/evento.json
python3 scripts/auditar.py --state "$HOME/.local/share/agentflix/editorial-visual" configure --policy /caminho/privado/politica.json
python3 scripts/auditar.py --state "$HOME/.local/share/agentflix/editorial-visual" audit
```
...
```sh
python3 scripts/auditar.py --state "$HOME/.local/share/agentflix/editorial-visual" ack --id ID_RETORNADO_NA_AUDITORIA
```
...
---
type: Playbook
title: Identidade visual para seus posts
description: Procedimento editorial autoral AgentFlix para a jornada de marca.
status: draft
generated:
  by: process:agentflix-skill-authoring
  at: 2026-09-10T01:48:21.011149+00:00
stale_after: 2026-12-09
sources:
  - id: procedimento
    resource: https://github.com/AgentsFlix/skills/tree/main/skills/editorial-visual
    title: Procedimento e recursos desta edição
agentflix:
  schema_version: 1
  skill_id: editorial-visual
...
## Referência: references/tokens.md

# Tokens de uma direção proposta

Use JSON com `brand`, `background`, `foreground`, `accent`, `font_family` e `approval_status` (`proposed` ou `approved`). Cores são hexadecimais #RRGGBB. Não use exemplo genérico como identidade confirmada.

Associe no brief origem de cada escolha, revisão, papel das cores, hierarquia e aprovação. A revisão exibida define a aprovação; mudar arquivos depois invalida a equivalência. Se usar editorial-templates, o helper confere contraste do texto e gera SVGs editáveis. Essa verificação mecânica não substitui a inspeção visual nem a aprovação da pessoa.

Cada aplicação ocupa 1080×1350 e tem margens internas. O arquivo reutilizável pode ter outro formato de edição, desde que exporte 4:5 e preserve tokens e conteúdos separados.
...
---
type: Skill Instance
title: Estado privado de Identidade visual para seus posts
status: draft
agentflix:
  schema_version: 1
  skill_id: editorial-visual
