...
const dashboardSandbox={module:{exports:{}},Date};
vm.runInNewContext(fs.readFileSync(path.join(dir,'jornada-base-dashboard.js'),'utf8'),dashboardSandbox);
const D=dashboardSandbox.module.exports;
...
const stage=(id,status='rascunho')=>({stage:id,status,summary:`Resumo de ${id}`,decisions:[id],sources:[id,'fonte'],pending:status==='rascunho'?['revisar']:[],documents:[`${id}.md`],updated_at:'2026-09-13'});
const bundle={format:'agentflix-base-bundle-1',business_name:'Marca de teste',created_at:'2026-09-13T00:00:00.000Z',records:{negocio:{filename:'negocio.json',output:stage('negocio','revisado')},pesquisa:{filename:'pesquisa.json',output:stage('pesquisa','revisado')},publico:{filename:'publico.json',output:stage('publico')},posicionamento:{filename:'posicionamento.json',output:stage('posicionamento','revisado')},voz:{filename:'voz.json',output:stage('voz')},'materia-prima':{filename:'materia-prima.json',output:stage('materia-prima')}}};
const dashboard=D.fromBundle(bundle);
assert(D.validProjection(dashboard));const totals=D.totals(dashboard);assert.equal(totals.reviewed,3);assert.equal(totals.decisions,6);assert.equal(totals.sources,12);assert.equal(totals.pending,3);
assert(M.validBaseDashboard(dashboard));
const individualDashboard=D.fromRecords(Object.values(bundle.records).map(record=>record.output));assert(D.validProjection(individualDashboard));assert(M.validBaseDashboard(individualDashboard));
assert.throws(()=>D.fromBundle({...bundle,records:{...bundle.records,extra:{}}}),/somente seis itens/);
assert.throws(()=>D.fromRecords(Object.values(bundle.records).slice(0,5).map(record=>record.output)),/pacote único ou os seis JSON/);
assert(!M.validBaseDashboard({...dashboard,records:dashboard.records.slice(0,5)}));
console.log('PASS jornada: prompts, continuidade, nove critérios, dois formatos, piloto, Drive declarado, dependências, histórico, isolamento por negócio e dashboard da base');
