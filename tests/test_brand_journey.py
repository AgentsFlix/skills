...
class BrandJourney(unittest.TestCase):
    ...
    def test_base_editorial_page_and_assets(self):
        ...
        self.assertIn('base-editorial-flow.js?v=20260915-4', page)
        ...
        self.assertIn('class="base-decision"', flow)
        self.assertIn("'-card.jpg/capa'", flow)
        self.assertIn('Skills usadas pelo agente nesta etapa', flow)
        self.assertIn('O Hermes consulta estas skills quando executar o prompt.', flow)
        self.assertIn("'maton-operations':'Maton AI'", flow)
        self.assertNotIn('Integração Hermes Maton', flow)
        ...
        self.assertIn("const guideKey='agentflix-ecf-onboarding-v3'", page_script)
        self.assertEqual(page_script.count("outcome:'Ao terminar"), 6)
        self.assertEqual(page_script.count("stageTitle:"), 6)
        self.assertEqual(page_script.count("choiceBody:"), 6)
        self.assertIn("guideButton.addEventListener('click'", page_script)
        self.assertIn("scheduleStageGuide(current)", page_script)
        self.assertIn("openGuide('stage',index)", page_script)
        self.assertIn("openGuide('choice',index)", page_script)
        self.assertIn("openGuide('copy',current)", page_script)
        self.assertIn("openGuide('return',current)", page_script)
        self.assertIn("openGuide('upload',current)", page_script)
        self.assertIn("querySelector('.base-decision')", page_script)
        self.assertIn("querySelector('[data-base-nav=\"'+stage.id+'\"]')", page_script)
        self.assertIn("classList.toggle('base-tour-nav-open',mode==='stage')", page_script)
        self.assertIn('function positionGuide()', page_script)
        self.assertIn("classList.add('base-tour-workspace-open')", page_script)
        self.assertIn("'Estou com o arquivo'", page_script)
        self.assertIn("ecf:prompt-copied", flow)
        self.assertIn("ecf:file-imported", flow)
        self.assertIn('shell.inert=modal', page_script)
        self.assertIn('shell.inert=false', page_script)
        self.assertIn('Ok, vamos lá', page_script)
        self.assertIn('No fim, seu conteúdo começa com clareza.', page_script)
        page_style = (directory / 'base-editorial.css').read_text()
        self.assertIn('.base-tour-backdrop', page_style)
        self.assertIn('.base-nav a.base-tour-target', page_style)
        self.assertIn('.base-nav.base-tour-nav-open a:not(.base-tour-target)', page_style)
        self.assertIn('box-shadow:inset 0 0 0 3px var(--accent)', page_style)
        self.assertIn('.base-tour-area-target', page_style)
        self.assertIn('.base-tour-action-target', page_style)
        self.assertIn('.base-workspace.base-tour-workspace-open', page_style)
    ...
    def test_brand_dashboard_uses_transparent_editorial_art(self):
        ...
        self.assertIn("style=\"--ideal-dot: '+target+'\"", script)
