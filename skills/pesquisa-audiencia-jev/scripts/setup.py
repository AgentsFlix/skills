...
def doctor(path):
    required = ["SKILL.md", "modules/jev-operar/GUIDE.md", "modules/jev-cerne/GUIDE.md",
                ...
                "scripts/setup.py", "requirements.txt"]
    missing = [name for name in required if not (ROOT / name).is_file()]
    return {"python_supported": sys.version_info >= (3, 10), "package_complete": all((ROOT / f).is_file() for f in required),
            "missing_package_files": missing, "yt_dlp_available": bool(jev.yt_dlp_executable()),
            "dependency_environment": str(jev.dependency_env_path()),
...
def install_deps(execute=False):
    ...
    if sys.version_info < (3, 10):
        raise jev.JevError("Python 3.10 or newer is required")
    if not (ROOT / "requirements.txt").is_file():
        raise jev.JevError("Package requirements are missing; reinstall the complete skill")
    ...
    if not executable.is_file() or not os.access(executable, os.X_OK):
        raise jev.JevError("yt-dlp unavailable after dependency installation")
...
def main(argv=None):
    ...
    except (jev.JevError, OSError, ValueError, subprocess.CalledProcessError) as exc:
